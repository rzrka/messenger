...
class ClientTransport(threading.Thread, QObject):
    ...
    def connection_init(self, port, ip):
        ...
        if not connected:
            logger.critical('Не удалось установаить соединение с сервером')
        ...
        with socket_lock:
            ...
            try:
                qmessage.send_msg(self.transport, presense)
                ans = qmessage.get_msg(self.transport)
                ...
                if RESPONSE in ans:
                    if ans[RESPONSE] == 400:
                        ...
                        my_ans[DATA] = binascii.b2a_base64(
                            digest).decode('ascii')
                        qmessage.send_msg(self.transport, my_ans)
                        self.process_server_ans(
                            qmessage.get_msg(self.transport))
    ...
    def contacts_list_update(self):
        ...
        with socket_lock:
            qmessage.send_msg(self.transport, req)
            ans = qmessage.get_msg(self.transport)
        logger.debug(f'Получено ответ {ans}')
    ...
    def add_contact(self, contact):
        ...
        with socket_lock:
            qmessage.send_msg(self.transport, req)
            self.process_server_ans(qmessage.get_msg(self.transport))
    ...
    def transport_shutdown(self):
        ...
        with socket_lock:
            try:
                qmessage.send_msg(self.transport, message)
            except OSError:
                pass
    ...
    def send_message(self, to, message):
        ...
        with socket_lock:
            qmessage.send_msg(self.transport, message_dict)
            self.process_server_ans(qmessage.get_msg(self.transport))
        ...
        while self.running:
            ...
            with socket_lock:
                try:
                    ...
                    message = qmessage.get_msg(self.transport)
                except OSError as err:
                    if err.errno:
                        logger.critical('Потеряно соединение с сервером.')
                        self.running = False
                    ...
                    logger.debug('Потеряно соединение с сервром.')
                    self.running = False
                    self.connection_lost.emit()
                finally:
                    self.transport.settimeout(5)
