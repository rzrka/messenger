...
from PyQt5.QtGui import QStandardItemModel, QStandardItem, QBrush, QColor, QKeyEvent
        ...
        if sender == self.current_chat:
            ...
            if self.database.check_contact(sender):
                ...
                if self.messages.question(
                    self,
                    'Новое сообщение',
                    f'Получено новое сообщение от {sender}.\n Данного пользователя нет в контактах.\n /'
    ...
    def sig_205(self):
        if self.current_chat and not self.database.check_user(
            ...
            self.messages.warning(
                self,
                'Сочуствую',
                'К сожалению собеседник был удален с сервера')
            self.set_disabled_input()
            self.current_chat = None
        self.clients_list_update()
